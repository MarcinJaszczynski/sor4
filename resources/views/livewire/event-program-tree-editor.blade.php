<div>
            ...
            <div class="bg-white shadow rounded-lg p-4 fi-section-content" data-day="{{ $dayNumber }}">
                ...
                <ul class="program-day-list space-y-2 min-h-[100px] border border-dashed border-gray-300 p-2 rounded-md"
                    ...
                    @forelse ($points as $point)
                        <li wire:key="wrapper-{{ $point['pivot_id'] ?? $point['id'] }}" class="program-point-wrapper mb-2" data-pivot-id="{{ $point['pivot_id'] ?? $point['id'] }}">
                            <div class="program-point-item group flex items-stretch bg-white rounded-2xl shadow-md border border-gray-200 hover:shadow-lg transition overflow-hidden relative"
                                ...
                                <div class="flex-1 flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white px-6 py-4">
                                    <div class="flex-1 min-w-0">
                                        <div class="text-lg font-semibold text-gray-800 break-words">{{ $point['name'] }}</div>
                                    ...
                                    <div class="w-20 pl-2 py-1 flex items-center space-x-2 self-center flex-shrink-0">
                                        <button wire:click="openTaskModal({{ $point['id'] }})"
                                            class="text-gray-400 hover:text-yellow-600 p-1" title="Zadania">
                                ...
                                <div class="children-container">
                                    <ul class="ml-8 mt-1">
                                        @foreach($point['children'] as $child)
                                            <li wire:key="child-{{ $child['id'] }}" class="bg-gray-100 p-3 rounded-md shadow-sm border border-gray-100 flex items-start">
    ...
    <!-- Task Modal -->
    <div x-data="{ show: @entangle('showTaskModal') }" x-show="show" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="show" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full sm:p-6">
                ...
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Zadania dla punktu programu</h3>
                ...
                <div class="bg-gray-50 p-4 rounded-lg mb-6">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <div>
                            <input type="text" wire:model="taskModalData.title" placeholder="Tytuł zadania" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                            @error('taskModalData.title') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <textarea wire:model="taskModalData.description" placeholder="Opis (opcjonalnie)" rows="2" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm"></textarea>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <select wire:model="taskModalData.status_id" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                                @foreach(\App\Models\TaskStatus::all() as $status)
                                    <option value="{{ $status->id }}">{{ $status->name }}</option>
                                @endforeach
                            </select>
                            <select wire:model="taskModalData.assignee_id" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                ...
                <div class="space-y-3 max-h-96 overflow-y-auto">
                    @forelse($tasksForCurrentPoint as $task)
                    ...
                    @empty
                        <p class="text-center text-gray-500 text-sm py-4">Brak zadań dla tego punktu.</p>
                    @endforelse
                </div>
...
<script>
    ...
    function destroyAllSortables() {
        if (!window.sortableInstances) return;
    ...
    function initializeSortable() {
        ...
        // Don't reinitialize if we are currently dragging
        if (document.body.classList.contains('sortable-drag-active')) {
            return;
        }
        ...
        dayLists.forEach((listEl) => {
            try {
                const sortableInstance = new Sortable(listEl, {
                    ...
                    animation: 150,
                    handle: '.drag-handle',
                    draggable: '.program-point-wrapper',
                    ghostClass: 'sortable-ghost',
                    chosenClass: 'sortable-chosen',
                    dragClass: 'sortable-drag',
                    dataIdAttr: 'data-pivot-id',
                    swapThreshold: 0.65,
                    invertSwap: false,
                    direction: 'vertical',
                    ...
                    revertOnSpill: true, // Revert if dropped outside
                    ...
                    onMove: function(evt) {
                        // Prevent error if target is null
                    ...
                    onEnd: function (evt) {
                        ...
                        daysContainer.querySelectorAll('.program-day-list').forEach(dayList => {
                            ...
                            dayList.querySelectorAll('.program-point-wrapper').forEach(item => {
                        ...
                        if (component) {
                            ...
                            console.error('Sortable onEnd error:', error);
                            setTimeout(() => {
                                initializeSortable();
                            }, 500);
                        }
