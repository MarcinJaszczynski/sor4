<div>
    ...
    <div id="notifications" class="fixed top-4 right-4 z-50">
        ...
        <h2 class="text-xl font-bold">Program imprezy: {{ $event->name }}</h2>
    ...
    <div class="grid grid-cols-1 gap-6" id="program-days-container" wire:init="load">
        @if(!$loaded)
            <div class="p-6 bg-white rounded shadow text-center">
                <div class="animate-spin h-6 w-6 border-2 border-gray-300 border-t-primary-600 rounded-full mx-auto mb-2"></div>
                <div class="text-gray-600">Ładowanie programu...</div>
            </div>
        @else
            @forelse ($programByDays as $dayData)
                <div class="bg-white shadow rounded-lg p-4 fi-section-content" data-day="{{ $dayData['day'] }}">
                    <div class="mb-3">
                        <h3 class="text-lg font-semibold text-gray-700">
                            @if($dayData['day'] > $event->duration_days)
                                Fakultatywnie
                            @else
                                Dzień {{ $dayData['day'] }}
                            @endif
                    ...
                    <ul class="program-day-list space-y-2 min-h-[100px] border border-dashed border-gray-300 p-2 rounded-md"
                        data-day-id="{{ $dayData['day'] }}">
                        @forelse ($dayData['points'] as $point)
                        <li wire:key="wrapper-{{ $point['id'] }}" class="program-point-wrapper mb-2" data-pivot-id="{{ $point['id'] }}">
                            <div class="program-point-item group flex items-stretch bg-white rounded-2xl shadow-md border border-gray-200 hover:shadow-lg transition overflow-hidden relative"
                                data-pivot-id="{{ $point['id'] }}" data-point-id="{{ $point['id'] }}" data-id="{{ $point['id'] }}">
                                <!-- Drag handle -->
                                ...
                                <div class="flex-1 flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white px-6 py-4">
                                    ...
                                    <div class="flex flex-row gap-3 items-center mt-2 md:mt-0">
                                        <label class="flex flex-col items-center cursor-pointer group">
                                            <input type="checkbox" 
                                                wire:click="togglePivotProperty({{ $point['id'] }}, 'include_in_program')" 
                                        ...
                                        <label class="flex flex-col items-center cursor-pointer group">
                                            <input type="checkbox" 
                                                wire:click="togglePivotProperty({{ $point['id'] }}, 'include_in_calculation')" 
                                        ...
                                        <label class="flex flex-col items-center cursor-pointer group">
                                            <input type="checkbox" 
                                                wire:click="togglePivotProperty({{ $point['id'] }}, 'active')" 
                                        ...
                                        <label class="flex flex-col items-center cursor-pointer group">
                                            <input type="checkbox" 
                                                wire:click="togglePivotProperty({{ $point['id'] }}, 'show_title_style')" 
                                        ...
                                        <label class="flex flex-col items-center cursor-pointer group">
                                            <input type="checkbox" 
                                                wire:click="togglePivotProperty({{ $point['id'] }}, 'show_description')" 
                                    ...
                                    <div class="w-20 pl-2 py-1 flex items-center space-x-2 self-center flex-shrink-0">
                                        <button wire:click="openTaskModal({{ $point['id'] }})" class="text-gray-500 hover:text-primary-600 transition-colors" title="Zadania">
                                            <x-heroicon-o-clipboard-document-check class="w-5 h-5" />
                                        </button>
                                        <button
                                            wire:click="showEditModal({{ $point['id'] }})"
                                            class="text-gray-400 hover:text-primary-600 p-1" title="Edytuj">
                                            <x-heroicon-o-pencil-square class="w-5 h-5" />
                                        </button>
                                        <button wire:click="duplicatePoint({{ $point['id'] }})"
                            ...
                            @if(!empty($point['children']))
                                <div class="children-container">
                                    <ul class="ml-8 mt-1">
                                        ...
                                        <!-- Col 5: Actions -->
                                        <div class="w-20 pl-2 py-1 flex items-center space-x-2 self-center flex-shrink-0">
                                            <button wire:click="openTaskModal({{ $child['id'] }})" class="text-gray-500 hover:text-primary-600 transition-colors" title="Zadania">
                                                <x-heroicon-o-clipboard-document-check class="w-5 h-5" />
                                            </button>
                                        </div>
