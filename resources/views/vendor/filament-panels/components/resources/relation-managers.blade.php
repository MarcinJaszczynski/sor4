    ...
    @if (filled($activeManager) && isset($managers[$activeManager]))
        ...
        >
            @php
                ...
                // Check strictly for the Finance group by label
                $isFinanceGroup = ($managers[$activeManager] instanceof \Filament\Resources\RelationManagers\RelationGroup)
                    && ($managers[$activeManager]->getLabel() === 'Finanse');
            ...
            @if ($managers[$activeManager] instanceof \Filament\Resources\RelationManagers\RelationGroup)
                @if ($isFinanceGroup)
                    {{-- Grid Layout for Finances --}}
                    
                    {{-- Embed the Calculation/Price Table Widget above the tables --}}
                    <div class="mb-6 w-full">
                        @livewire(\App\Filament\Resources\EventResource\Widgets\EventPriceTable::class, ['record' => $ownerRecord])
                    </div>

                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full items-start" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(450px, 1fr));">
                        @foreach ($managers[$activeManager]->ownerRecord($ownerRecord)->pageClass($pageClass)->getManagers() as $groupedManagerKey => $groupedManager)
                            ...
                            <div class="min-w-0 w-full" wire:key="wrapper-{{ $normalizedGroupedManagerClass }}-{{ $groupedManagerKey }}">
                                @livewire(
                ...
                @else
                    {{-- Standard Vertical Layout for other groups --}}
                    @foreach ($managers[$activeManager]->ownerRecord($ownerRecord)->pageClass($pageClass)->getManagers() as $groupedManagerKey => $groupedManager)
                        @php
                            $normalizedGroupedManagerClass = $normalizeRelationManagerClass($groupedManager);
                        @endphp
