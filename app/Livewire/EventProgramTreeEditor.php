...
class EventProgramTreeEditor extends Component
{
    ...
    {
        Log::info('updateProgramOrder called', ['list_count' => count($list)]);
        try {
            DB::beginTransaction();
            ...
            DB::commit();
            Log::info('updateProgramOrder committed');

            // Reload program data after reordering so UI reflects changes
            $this->loadProgramByDays();
    ...
    public function loadTasksForPoint()
    {
        if ($this->currentPointIdForTasks) {
            $this->tasksForCurrentPoint = Task::where('taskable_type', EventTemplateProgramPoint::class)
    ...
    public function saveTask()
    {
        ...
        $this->loadTasksForPoint();
        $this->resetTaskModalData();
        $this->dispatch('notify', ['message' => 'Zadanie zostało dodane', 'type' => 'success']);
    }

    public function deleteTask($taskId)
    {
        $task = Task::find($taskId);
        if ($task && $task->taskable_type === EventTemplateProgramPoint::class && $task->taskable_id == $this->currentPointIdForTasks) {
            $task->delete();
            $this->loadTasksForPoint();
            $this->dispatch('notify', ['message' => 'Zadanie zostało usunięte', 'type' => 'success']);
        }
    }
}
