...
class TasksKanbanBoardPage extends Page implements HasForms
{
    ...
    public $newComment = '';
    public $newCommentRecipientIds = [];
    public $replyContents = [];
    public $replyRecipientIds = [];
    ...
    {
        $query = Task::query()
            ->with(['status', 'assignee', 'author', 'subtasks', 'attachments', 'comments.author', 'comments.recipient', 'comments.recipients']);
    ...
    {
        $userId = Auth::id();
        $this->currentTaskForDetails = Task::with(['author', 'assignee', 'taskable'])->findOrFail($taskId);
        $this->showingComments = true;
        if ($this->currentTaskForDetails) {
            if ($userId && $this->currentTaskForDetails->assignee_id && (int) $userId !== (int) $this->currentTaskForDetails->assignee_id) {
                $this->newCommentRecipientIds = [$this->currentTaskForDetails->assignee_id];
            } elseif ($userId && $this->currentTaskForDetails->author_id && (int) $userId !== (int) $this->currentTaskForDetails->author_id) {
                $this->newCommentRecipientIds = [$this->currentTaskForDetails->author_id];
            } else {
                $this->newCommentRecipientIds = [];
            }
        }
        ...
        $comments = TaskComment::query()
            ...
            ->with([
                ...
                'replies' => fn ($q) => $q
                    ->visibleTo((int) ($userId ?? 0))
                    ->with(['author:id,name', 'recipient:id,name', 'recipients:id,name'])
                    ->orderBy('created_at'),
            ])
            ->orderBy('created_at')
            ->get();

        $this->currentTaskForDetails->setRelation('comments', $comments);
        $user = Auth::user();
        if ($user) {
    ...
    public function addComment()
    {
        ...
        try {
            ...
            $comment = $this->currentTaskForDetails->comments()->latest('id')->first();
            $recipientIds = collect($this->newCommentRecipientIds)
                ->filter()
                ->map(fn ($id) => (int) $id)
                ->unique()
                ->values();
            ...
            if ($recipientIds->isEmpty()) {
                if ($this->currentTaskForDetails->assignee_id && (int) Auth::id() !== (int) $this->currentTaskForDetails->assignee_id) {
                    $recipientIds = collect([$this->currentTaskForDetails->assignee_id]);
                } elseif ($this->currentTaskForDetails->author_id && (int) Auth::id() !== (int) $this->currentTaskForDetails->author_id) {
                    $recipientIds = collect([$this->currentTaskForDetails->author_id]);
                }
            }
            ...
            $this->currentTaskForDetails->load(['author', 'assignee', 'taskable']);
            $this->showComments($this->currentTaskForDetails->id);
    ...
    public function deleteComment($commentId)
    {
        try {
            ...
            $comment->delete();
            $this->currentTaskForDetails->refresh();
            $this->currentTaskForDetails->load(['comments.author', 'comments.recipient', 'comments.recipients', 'author', 'assignee']);
    ...
    public function addReply(int $commentId): void
    {
        ...
        $content = trim((string) ($this->replyContents[$commentId] ?? ''));

        if ($content === '') {
            return;
        }

        $reply = $comment->task->comments()->create([
            'content' => $content,
            'author_id' => Auth::id(),
            'parent_id' => $comment->id,
        ]);

        $recipientIds = collect($this->replyRecipientIds[$commentId] ?? [])
            ->filter()
            ->map(fn ($id) => (int) $id)
            ->unique()
            ->values();

        if ($reply && $recipientIds->isNotEmpty()) {
            $reply->recipients()->sync($recipientIds->all());
            $reply->recipient_id = $recipientIds->first();
            $reply->save();
        }

        app(TaskCommentNotificationService::class)->notify($reply, $recipientIds->all());

        $this->replyContents[$commentId] = '';
        $this->replyRecipientIds[$commentId] = [];
