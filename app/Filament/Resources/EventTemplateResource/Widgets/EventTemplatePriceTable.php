...
class EventTemplatePriceTable extends Widget
{
    ...
    public function getDetailedCalculations()
    {
        ...
        foreach ($qtyVariants as $qtyVariant) {
            ...
            $markupDetails = $this->calculateMarkupDetails($plnTotal);
            $markupCalculation = ['amount' => $markupDetails['amount']];
            ...
            $calculations[$qty]['markup'] = [
                ...
                'percent_applied' => $markupDetails['percent'],
                'discount_applied' => false, // uproszczona wersja - bez skomplikowanej logiki rabatów
                'discount_percent' => 0,
                'min_daily_applied' => $markupDetails['min_applied'] ?? false,
            ];
    ...
    {
        $details = $this->calculateMarkupDetails($basePrice);
        return $details['amount'];
    }

    /**
     * Zwraca szczegóły obliczenia narzutu: percentowa kwota, minimalna kwota (pomnożona przez dni),
     * wynikowa kwota oraz flaga czy minimalna została zastosowana.
     */
    private function calculateMarkupDetails($basePrice): array
    {
        $markupPercent = $this->getMarkupPercent();
        $percentAmount = $basePrice * ($markupPercent / 100);

        // pobierz minimalny narzut z powiązanego Markup lub z domyślnego
        $minDaily = 0;
        if (isset($this->record->markup) && $this->record->markup?->min_daily_amount_pln !== null) {
            $minDaily = (float) $this->record->markup->min_daily_amount_pln;
        ...
        $duration = $this->record->duration_days ?? 1;
        $minTotal = $minDaily * $duration;

        $final = $percentAmount;
        $minApplied = false;
        if ($minTotal > 0 && $percentAmount < $minTotal) {
            $final = $minTotal;
            $minApplied = true;
        }
