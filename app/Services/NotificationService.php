...
class NotificationService
{
    ...
    public static function getUnreadCountsForUser(int $userId): array
    {
        ...
        return Cache::remember($cacheKey, now()->addMinutes(2), function () use ($userId) {
            ...
            // Count email-share notifications (database) referencing email_message_id
            $emailsCount = \Illuminate\Support\Facades\DB::table('notifications')
                ->where('notifiable_id', $user->id)
                ->where(function ($q) {
                    $q->where('data', 'like', '%email_message_id%')
                      ->orWhere('type', 'App\\Notifications\\EmailSharedNotification');
                })
                ->count();

            // Unread task comments
            $taskCommentsCount = \Illuminate\Support\Facades\DB::table('notifications')
                ->where('notifiable_id', $user->id)
            ...
            return [
                ...
                'emails' => $emailsCount,
                'task_comments' => $taskCommentsCount,
            ];
        });
    }
