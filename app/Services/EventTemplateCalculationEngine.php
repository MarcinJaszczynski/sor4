...
class EventTemplateCalculationEngine
{
    ...
    private function getMarkupMinDailyForTemplate(EventTemplate $template): float
    {
        // Prefer related Markup model if loaded
        if (isset($template->markup) && $template->markup?->min_daily_amount_pln !== null) {
            return (float) $template->markup->min_daily_amount_pln;
        }

        // Try to resolve by markup_id
        if (!empty($template->markup_id)) {
            $m = \App\Models\Markup::find($template->markup_id);
            if ($m && $m->min_daily_amount_pln !== null) return (float) $m->min_daily_amount_pln;
        }

        // Fallback to default markup record
        $default = \App\Models\Markup::where('is_default', true)->first();
        return (float) ($default?->min_daily_amount_pln ?? 0);
    }
    ...
    private function calculateMarkupForTemplate(EventTemplate $template, float $base): float
    {
        ...
        $minDaily = $this->getMarkupMinDailyForTemplate($template);
        $duration = $template->duration_days ?? 1;
        $minTotal = $minDaily * $duration;

        if ($minTotal > 0 && $calculated < $minTotal) {
            \Illuminate\Support\Facades\Log::info("[MARKUP] Applying min daily markup: using {$minTotal} instead of calculated {$calculated} for event_template_id={$template->id}");
            return $minTotal;
        }
