...
class Event extends Model
{
    ...
    protected $casts = [
        ...
        'transport_price_override' => 'decimal:2',
        'custom_hotel_prices' => 'array',
    ];
    ...
    protected static function booted()
    {
        static::saving(function ($event) {
            try {
                ...
                $duration = (int) ($event->duration_days ?? 0);

                // If start + duration provided, compute end_date
                if ($start && $duration > 0) {
                    $startDt = \Illuminate\Support\Carbon::parse($start);
                    $event->end_date = $startDt->copy()->addDays(max(0, $duration - 1))->toDateString();
                }

                // If both start and end provided, ensure duration matches
                if ($start && ($event->end_date ?? null)) {
                    $startDt = \Illuminate\Support\Carbon::parse($start);
                    $endDt = \Illuminate\Support\Carbon::parse($event->end_date);
                    $event->duration_days = $startDt->diffInDays($endDt) + 1;
                }
            } catch (\Throwable $_) {
                // ignore
            }
        ...
        static::creating(function ($event) {
            ...
            // Ensure there is a unique public code for the event
            if (empty($event->public_code)) {
                do {
    ...
    {
        return $this->hasMany(EventPaymentCostAllocation::class);
    }
    ...
    /**
     * Wydatki pilota powiązane z imprezą
     */
    public function pilotExpenses(): HasMany
    {
        return $this->hasMany(PilotExpense::class);
    }
    ...
    /**
     * Dni hotelowe przypisane do imprezy
     */
    public function hotelDays(): HasMany
    {
        return $this->hasMany(EventHotelDay::class);
    }
    ...
    public static function createFromTemplate(EventTemplate $template, array $data): self
    {
        ...
        $event = self::create([
            ...
            'start_date' => $data['start_date'] ?? now()->addDays(14)->toDateString(),
            ...
            'participant_count' => (int)($data['participant_count'] ?? 1),
            'gratis_count' => (int)($data['gratis_count'] ?? 0),
            'staff_count' => (int)($data['staff_count'] ?? 0),
            'driver_count' => (int)($data['driver_count'] ?? 0),
            'guide_count' => (int)($data['guide_count'] ?? 0),
        ...
        // Synchronizuj pilotów jeśli zostali wybrani
        if (isset($data['guides'])) {
            $event->guides()->sync($data['guides']);
        }
        ...
        // Kopiuj dni hotelowe
        try {
            foreach ($template->hotelDays()->get() as $hd) {
                \App\Models\EventHotelDay::create([
                    'event_id' => $event->id,
                    'day' => $hd->day,
                    'hotel_room_ids_qty' => $hd->hotel_room_ids_qty,
                    'hotel_room_ids_gratis' => $hd->hotel_room_ids_gratis,
                    'hotel_room_ids_staff' => $hd->hotel_room_ids_staff,
                    'hotel_room_ids_driver' => $hd->hotel_room_ids_driver,
                    'notes' => $hd->notes,
                ]);
            }
        } catch (\Throwable $e) {
    ...
    public function copyProgramPointsFromTemplate(): void
    {
        ...
        foreach ($templatePoints as $point) {
            ...
            $mainPoint = EventProgramPoint::create([
                ...
                'quantity' => 1,
                'total_price' => $point->unit_price ?? 0,
            ...
            // Skopiuj podpunkty jako dzieci (z parent_id)
            if ($point->children && $point->children->count() > 0) {
                ...
                foreach ($point->children as $child) {
                    $childPoint = EventProgramPoint::create([
                        ...
                        'day' => $point->pivot->day, // Dzieci mają ten sam dzień co rodzic
                        'order' => $childOrder,
                        ...
                        'quantity' => 1,
                        'total_price' => $child->unit_price ?? 0,
    ...
    public function calculateTotalCost(): void
    {
        ...
        $calculation = $engine->calculate($this);

        $this->update(['total_cost' => $calculation['total_cost']]);
    }
    ...
    /**
     * Umowy wygenerowane dla imprezy
     */
    ...
    /**
     * Płatności od klienta (przychody)
     */
